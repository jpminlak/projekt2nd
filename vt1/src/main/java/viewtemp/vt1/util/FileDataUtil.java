...
//    @Resource(name = "uploadPath")    // 컨테이너에 있는 객체의 이름이 uploadPath라는 주소를 주입(DI)받는다.
...
//    @ResponseBody
//    // 어떤 데이터를 포함하여 전송.. 어노테이션.. view지정하지 않고 바로 클라이언트 요청으로 응답. return 타입이 뷰가 아닌 리턴타입 그 자체로 응답. 리턴타입은 파일 시스템(바이너리 파일).
